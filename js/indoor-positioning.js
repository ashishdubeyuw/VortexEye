...
class IndoorPositioningService {
    ...
    /**
     * Update position based on step counter + heading (dead reckoning)
     * @param {object} stepData - { stepCount, distanceMeters }
     * @param {number} heading - Compass heading in degrees (0=N, 90=E, 180=S, 270=W)
     */
    updatePosition(stepData, heading) {
        this.heading = heading;

        if (!stepData || !stepData.distanceMeters) return;

        // Calculate movement vector from heading
        const headingRad = (heading * Math.PI) / 180;
        const dx = stepData.distanceMeters * Math.sin(headingRad); // East-West
        const dy = -stepData.distanceMeters * Math.cos(headingRad); // North-South (inverted for grid)

        // Accumulate offset from anchor
        this.estimatedOffset.x += dx;
        this.estimatedOffset.y += dy;

        // Convert offset to quadrant coordinates
        const newCol = Math.floor(this.estimatedOffset.x / this.cellSizeMeters);
        const newRow = Math.floor(this.estimatedOffset.y / this.cellSizeMeters);
        ...
        // Check if we moved to a new quadrant
        if (clampedRow !== this.currentQuadrant.row || clampedCol !== this.currentQuadrant.col) {
            const targetQ = this.grid[clampedRow][clampedCol];
            if (targetQ && targetQ.walkable) {
                this.currentQuadrant = { row: clampedRow, col: clampedCol };
                console.log(`📍 Moved to quadrant Q(${clampedCol},${clampedRow}): ${targetQ.label || 'unknown'}`);
                this.notifyListeners('quadrantChange', this.currentQuadrant);

                // Check if we reached route waypoint
    ...
    correctWithDetection(detection) {
        ...
        if (closestQ && closestDist <= 3) {
            ...
            this.estimatedOffset = {
                x: closestQ.col * this.cellSizeMeters,
                y: closestQ.row * this.cellSizeMeters
            };
    ...
    updateFromBluetooth(bleData) {
        ...
        if (col >= 0 && col < this.gridCols && row >= 0 && row < this.gridRows) {
            ...
            // Just like camera correction, we snap/fuse this position
            // In a real system we might use a Kalman Filter, but here we trust BLE if it changes the quadrant
            if (newQ.row !== this.currentQuadrant.row || newQ.col !== this.currentQuadrant.col) {
                this.currentQuadrant = newQ;

                // Update internal offset to match new quadrant center
                this.estimatedOffset = {
                    x: col * this.cellSizeMeters,
                    y: row * this.cellSizeMeters
                };

                console.log(`🔵 BLE Update → Q(${col},${row})`);
                this.notifyListeners('quadrantChange', this.currentQuadrant);
                this.checkRouteProgress();
            }
        }
